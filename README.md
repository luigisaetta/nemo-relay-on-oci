...
| [Order fulfillment — Responses API](demos/order_fulfillment_responses/README.md) | Independent copy of the order workflow using the official OpenAI SDK against OCI Responses API, `oci-genai-auth`, and Relay-managed `llm.execute` with native token, pricing, PII-redaction, and guardrail behavior. |
